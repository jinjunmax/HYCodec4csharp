...
{
    public class HYEncode : HYFontBase
    {  
        ...
        public HYRESULT FontOpen(string strFileName)
        {
            try
            {
                FRStrm = new FileStream(strFileName, FileMode.Create, FileAccess.ReadWrite, FileShare.None);
            }
            catch
            {
                throw;
        ...
        public HYRESULT EncodeFont(string strFontPath, string strProFile, UInt32 TableFlag, ref CharsInfo encdChars)
        {
            ...
            FRStrm = new FileStream(strFontPath,FileMode.Create,FileAccess.ReadWrite,FileShare.None);
            ...
            {
                LoadTypographicFile(strProFile, codemap.lstCodeMap);
                EncodeGSUB();             
            } 
            ...
            for (int i=0; i<TableDirectorty.numTables; i++)
		    {
                ...
                FRStrm.Seek(HYEntry.offset,SeekOrigin.Begin);
                FRStrm.Read(pCheckBuf,0,(int)CheckBufSz);
        ...
        protected HYRESULT LoadTypographicFile(string Typographicfile, List<HYCodeMapItem> lstCodeMap)
        {
            Gsub = new CGSUB();
            ...
            XmlReader reader = XmlReader.Create(Typographicfile, settings);
            xmlDoc.Load(reader);
            loadGsub(xmlDoc,lstCodeMap);
            
            return HYRESULT.NOERROR;

        }   // end of protected HYRESULT LoadTypographicfileFile()        

        protected void loadGsub(XmlDocument xmlDoc, List<HYCodeMapItem> lstCodeMap)
        {
            // 得到scriptlist
            XmlNode xScriptRoot = xmlDoc.SelectSingleNode("FontProfile/GSUB/scriptlist");
            if (xScriptRoot != null)
            {
                foreach (XmlNode xn in xScriptRoot.ChildNodes)
                {
                    CScriptRecord scrRecord = new CScriptRecord();
                    scrRecord.Tag = System.Text.Encoding.Default.GetBytes(xn.Attributes["tag"].Value);

                    XmlNodeList xnlst = xn.ChildNodes;
                    foreach (XmlNode xn1 in xnlst)
                    {
                        if (xn1.Name == "DefaultLangSys")
                        {
                            scrRecord.Script.DefaultLangSys.ReqFeatureIndex = UInt16.Parse(xn1.Attributes["ReqFeatureIndex"].Value);
                            foreach (XmlNode xNode in xn1.ChildNodes)
                            {
                                UInt16 Indx = UInt16.Parse(xNode.InnerText);
                                scrRecord.Script.DefaultLangSys.vtFeatureIndex.Add(Indx);
                            }
                        }

                        if (xn1.Name == "LangSysTable")
                        {
                            CLangSysRecord sysRecord = new CLangSysRecord();
                            sysRecord.SysTag = System.Text.Encoding.Default.GetBytes(xn1.Attributes["tag"].Value);
                            sysRecord.LangSys.ReqFeatureIndex = UInt16.Parse(xn1.Attributes["ReqFeatureIndex"].Value);

                            foreach (XmlNode xNode in xn1.ChildNodes)
                            {
                                UInt16 Indx = UInt16.Parse(xNode.InnerText);
                                sysRecord.LangSys.vtFeatureIndex.Add(Indx);
                            }
                            scrRecord.Script.vtLangSysRecord.Add(sysRecord);
                        }
                    }
                    Gsub.vtScriptsList.Add(scrRecord);
                }
            }

            // 得到featuretable
            XmlNode xfeatureRoot = xmlDoc.SelectSingleNode("FontProfile/GSUB/featurelist");
            if (xfeatureRoot != null)
            {
                foreach (XmlNode xfeaturetable in xfeatureRoot.ChildNodes)
                {
                    CFeatureRecord FeatureRecord = new CFeatureRecord();
                    FeatureRecord.Tag = System.Text.Encoding.Default.GetBytes(xfeaturetable.Attributes["tag"].Value);
                    foreach (XmlNode xNode in xfeaturetable.ChildNodes)
                    {
                        UInt16 Indx = UInt16.Parse(xNode.InnerText);
                        FeatureRecord.FeatureTable.vtLookupListIndex.Add(Indx);
                    }

                    Gsub.vtFeaturesList.Add(FeatureRecord);
                }
            }

            //得到lookuplist
            XmlNode xlookupRoot = xmlDoc.SelectSingleNode("FontProfile/GSUB/lookuplist");
            if (xlookupRoot != null)
            {
                foreach (XmlNode xlookup in xlookupRoot.ChildNodes)
                {
                    CLookUp lookup = new CLookUp();
                    lookup.LookUpType = UInt16.Parse(xlookup.Attributes["type"].Value);
                    lookup.LookUpFlag = UInt16.Parse(xlookup.Attributes["flag"].Value);

                    // 目前仅仅支持LookupType 1
                    if (lookup.LookUpType == 1)
                    {
                        foreach (XmlNode xSubTable in xlookup.ChildNodes)
                        {
                            CLookUpType1 lookupType1 = new CLookUpType1();

                            //  目前仅仅支持SubFormat = 2
                            lookupType1.SubFormat = UInt16.Parse(xSubTable.Attributes["SubstFormat"].Value);
                            if (lookupType1.SubFormat == 2)
                            {
                                ...
                                foreach (XmlNode xChar in xSubTable.ChildNodes)
                                {
                                    string strcontent = xChar.InnerText;
                                    string delimStr = ",";
                                    char[] delimiter = delimStr.ToCharArray();
                                    string[] split = strcontent.Split(delimiter, 2);
                                    ...
                                    if (split.Length == 2)
                                    {
                                        ...
                                        if (str1[0] == 'G')
                                        {
                                            string strsub = split[0].Substring(1);
                                            UInt16 Gid = UInt16.Parse(strsub);
                                            Coverage.vtGlyphID.Add(Gid);
                                        }

                                        if (str1[0] == 'U')
                                        {
                                            string strsub = split[0].Substring(1);
                                            UInt32 ulUnicode = UInt32.Parse(strsub, System.Globalization.NumberStyles.AllowHexSpecifier);
                                            UInt16 Gid = (UInt16)FindGryphIndexByUnciodeEx(ulUnicode, lstCodeMap);
                                            Coverage.vtGlyphID.Add(Gid);
                                            Coverage.GlyphCount++;
                                        }
                                        ...
                                        if (str2[0] == 'G')
                                        {
                                            string strsub = split[1].Substring(1);
                                            UInt16 Gid = UInt16.Parse(strsub);
                                            SSubstitution2.vtGlyphID.Add(Gid);
                                        }
                                    ...
                                    SSubstitution2.GlyphCount++;
                                }
                            }
                            
                            lookup.vtLookupType1.Add(lookupType1);
                        }
                    }
                    Gsub.vtLookupList.Add(lookup);
                }
            }     
        
        }   // end of protected void loadGsub()
        ...
        }   // end of protected int MakeTableDirectory()

        protected HYRESULT CountCodeMap()
        {
        ...
        }   // end of protected HYRESULT CountCodeMap()

        protected HYRESULT MakeCmap()
        {
            Cmap = new CCmap();
        ...
        {            
            ...
            FRStrm.Seek(0,SeekOrigin.Begin);
            ...
            FRStrm.Write(btTmp,0,btTmp.Length);
        ...
        {            
            ...
            tbEntry.offset = (uint)FRStrm.Position;
            ...
            long ulCmapEndPos = FRStrm.Position;
            FRStrm.Seek(tbEntry.offset+4,SeekOrigin.Begin);
            ...
            FRStrm.Flush();
            FRStrm.Seek(ulCmapEndPos,SeekOrigin.Begin);
            tbEntry.length = (uint)FRStrm.Position - tbEntry.offset;
        ...
        public HYRESULT Encodehead()
        {
            ...
            if (Head.created == null)
            {
                for (int i = 0; i < 8; i++)
                {
                    FRStrm.WriteByte(btTmp[7 - i]);
                }
            }
            else
            {
                FRStrm.Write(Head.created,0,8);
            }
        ...
        public HYRESULT Encodename()
        {
            ...
            FRStrm.Write(StrContent, 0, usStringSize);
        ...
        public HYRESULT EncodeOS2(List<HYCodeMapItem> lstCodeMap,UInt32 TableFlag)
        {
            ...
            //Panose            
            FRStrm.WriteByte(OS2.panose.FamilyType);
            FRStrm.WriteByte(OS2.panose.SerifStyle);
            FRStrm.WriteByte(OS2.panose.Weight);
            FRStrm.WriteByte(OS2.panose.Proportion);
            FRStrm.WriteByte(OS2.panose.Contrast);
            FRStrm.WriteByte(OS2.panose.StrokeVariation);
            FRStrm.WriteByte(OS2.panose.ArmStyle);
            FRStrm.WriteByte(OS2.panose.Letterform);
            FRStrm.WriteByte(OS2.panose.Midline);
            FRStrm.WriteByte(OS2.panose.XHeight);
            ...
            FRStrm.Write(btTmp, 0, btTmp.Length);
			//achVendID
            FRStrm.WriteByte(OS2.vtachVendID[0]);
            FRStrm.WriteByte(OS2.vtachVendID[1]);
            FRStrm.WriteByte(OS2.vtachVendID[2]);
            FRStrm.WriteByte(OS2.vtachVendID[3]);						
			//fsSelection
        ...
        public HYRESULT Encodepost()
        {
            ...
            if (Post.version.value == 2 && Post.version.fract == 0)
			{
				...
				for (int i = 258; i <st; i++)
				{
                    ...
                    FRStrm.WriteByte(strlen);
                    FRStrm.Write(szString, 0, strlen);
				}
			}
        ...
        public HYRESULT EncodeCFF()
        {
            ...
            FRStrm.Write(btHead, 0, btHead.Length);
			
			// Name
            byte[] btName = vtName.ToArray();
            FRStrm.Write(btName, 0, btName.Length);

			// TopDict
            byte[] btTopDict = vtTopDict.ToArray();
            FRStrm.Write(btTopDict, 0, btTopDict.Length);

			// string
            byte[] btString = vtString.ToArray();
            FRStrm.Write(btString, 0, btString.Length);

			// Global
            byte[] btGolbIndex = vtGolbIndex.ToArray();
            FRStrm.Write(btGolbIndex, 0, btGolbIndex.Length);
			
			// charset
            byte[] btCharset = vtCharset.ToArray();
            FRStrm.Write(btCharset, 0, btCharset.Length);

			if (CFFInfo.TopDICT.IsCIDFont==1)
			{
				// FDSelect
                byte[] btFDSelect = vtFDSelect.ToArray();
                FRStrm.Write(btFDSelect, 0, btFDSelect.Length);
			}

            byte[] btCharStrings = vtCharStrings.ToArray();
            FRStrm.Write(btCharStrings, 0, btCharStrings.Length);

            if (CFFInfo.TopDICT.IsCIDFont == 1)
            {
                //fdarray			
                byte[] btFArrayData = vtFArrayData.ToArray();
                FRStrm.Write(btFArrayData, 0, btFArrayData.Length);
            }
            else
            {
                //private Dict
                byte[] btPrivateData = vtPrivateData.ToArray();
                FRStrm.Write(btPrivateData, 0, btPrivateData.Length);
            }
    ...
    //			vtTmpData.Add(byte(CFF_DICT_OPERATOR_ROS));
        ...
        public HYRESULT EncodeGlyph()
        {
            ...
            long lTableBegin = FRStrm.Position;
            ...
            for (int i = 0; i < GlyphChars.CharInfo.Count; i++)
            {
                ...
                ulGlyphOffset = (uint)(FRStrm.Position - lTableBegin);
            ...
            tbEntry.length = (uint)FRStrm.Position - tbEntry.offset;
            int iTail1 = 4 - (int)tbEntry.length % 4;
            if (tbEntry.length % 4 > 0)
            {
                byte c = 0;
                for (int i = 0; i < iTail1; i++)
                {
                    FRStrm.WriteByte(c);
                }
            }
        ...
        protected HYRESULT EncodeScriptList(long GsubBeginPos, ref long GsubEndPos)
        {
			...
			long ulScriptLstBeginPos = FRStrm.Position;

			//write ScriptList offset
            FRStrm.Seek(GsubBeginPos+4,SeekOrigin.Begin);		
			usTmp = hy_cdr_int16_to((UInt16)(ulScriptLstBeginPos-GsubBeginPos));
            btTmp = BitConverter.GetBytes(usTmp);
            FRStrm.Write(btTmp, 0, btTmp.Length);

			FRStrm.Seek(ulScriptLstBeginPos,SeekOrigin.Begin);
			...
			for (int i=0; i<stScriptsList; i++)
			{
				...
				//ScriptTag
                FRStrm.WriteByte(ScriptRecord.Tag[0]);	
				FRStrm.WriteByte(ScriptRecord.Tag[1]);	
                FRStrm.WriteByte(ScriptRecord.Tag[2]);	
                FRStrm.WriteByte(ScriptRecord.Tag[3]);
			...
			{
				// save Script Offset
				long ulScriptBegin = FRStrm.Position;
				...
				for (int j=0; j<stLangSysRecord; j++)
				{
					...
					//LangSysRecord Tag
                    FRStrm.WriteByte(LangSysRecord.SysTag[0]);
                    FRStrm.WriteByte(LangSysRecord.SysTag[1]);
                    FRStrm.WriteByte(LangSysRecord.SysTag[2]);
                    FRStrm.WriteByte(LangSysRecord.SysTag[3]);
				...
				for (int j=0; j<stLangSysRecord; j++)
				{
					long ulLangSysBegin = FRStrm.Position;
				...
				GsubEndPos = FRStrm.Position;

				// write LangSys offset
                FRStrm.Seek(ulScriptBegin+4,SeekOrigin.Begin);
                ...
                FRStrm.Seek(GsubEndPos,SeekOrigin.Begin);			
				...
				{	
                    FRStrm.Seek(ulScriptBegin,SeekOrigin.Begin);
			...
			// write scriptlist  offset			
            FRStrm.Seek(ulScriptLstBeginPos + 2,SeekOrigin.Begin);
            ...
            FRStrm.Seek(GsubEndPos + 2, SeekOrigin.Begin);
        ...
        protected HYRESULT EncodeFeatureList(long GsubBeginPos, ref long GsubEndPos)
        {
			...
			long ulFeatureLstBeginPos = FRStrm.Position;

			//write FeatureList offset			
            FRStrm.Seek(GsubBeginPos+6,SeekOrigin.Begin);

			usTmp = hy_cdr_int16_to((UInt16)(ulFeatureLstBeginPos-GsubBeginPos));
			btTmp = BitConverter.GetBytes(usTmp);
            FRStrm.Write(btTmp, 0, btTmp.Length);
			FRStrm.Seek(ulFeatureLstBeginPos,SeekOrigin.Begin);
			...
			for (int i=0; i<stFeatureList; i++)
			{
				...
				//FeatureTag
                FRStrm.WriteByte(FeaturTable.Tag[0]);
                FRStrm.WriteByte(FeaturTable.Tag[1]);
                FRStrm.WriteByte(FeaturTable.Tag[2]);
                FRStrm.WriteByte(FeaturTable.Tag[3]);
			...
			for (int i=0; i<stFeatureList; i++)
			{
				...
				//  Feature offset
				vtFeatureOffset.Add(FRStrm.Position-ulFeatureLstBeginPos);
			...
			{
				CFeatureRecord FeaturTable = Gsub.vtFeaturesList[i];
                FRStrm.Seek(ltmp,SeekOrigin.Begin);
        ...
        protected HYRESULT EncodeLookupList(long GsubBeginPos, ref long GsubEndPos)
        {
            ...
            long lLookupLstBegin = FRStrm.Position;

            //write FeatureList offset
            FRStrm.Seek(GsubBeginPos+8,SeekOrigin.Begin);
            usTmp = hy_cdr_int16_to((UInt16)(lLookupLstBegin-GsubBeginPos));
            btTmp = BitConverter.GetBytes(usTmp);
            FRStrm.Write(btTmp, 0, btTmp.Length);

            FRStrm.Seek(lLookupLstBegin, SeekOrigin.Begin);
            ...
            for (int i = 0; i < szLookUpList; i++)
            {
                ...
                long lLookUpBegin = FRStrm.Position;
                ...
                if (lookup.LookUpType == 1)
                {
                    for (int j = 0; j < lookup.vtLookupType1.Count; j++)
                    {
                        ...
                        long lSubstitutionBegin = FRStrm.Position;
                        // save  SubTables offset from beginning of Lookup table 
                        ...
                        if (LookUpType1.SubFormat == 2)
                        {
                            ...
                            FRStrm.Seek(lSubstitutionBegin + 2,SeekOrigin.Begin);
                ...
                FRStrm.Seek(lLookUpBegin+6,SeekOrigin.Begin);
            ...
            FRStrm.Seek(lLookupLstBegin + 2, SeekOrigin.Begin);
        ...
        public HYRESULT Encodevhea()
        {
            ...
            CTableEntry tbEntry = TableDirectorty.vtTableEntry[iEntryIndex];
			tbEntry.offset = (uint)this.FRStrm.Position;
        ...
        public HYRESULT Encodevmtx()
        {
            ...
            for (int i = 0; i < Maxp.numGlyphs; i++)
            {
                if (i < Vhea.numOfLongVerMetrics)
                {
                    ...
                    FRStrm.Write(bTmp, 0, bTmp.Length);
        ...
        public HYRESULT EndcodePrep()
        {
            ...
            FRStrm.Write(PreData,0,PreData.Length);
	    ...
	    {		    
            ...
            long lTMP = FRStrm.Position;
			
            FRStrm.Seek(bGROffset,SeekOrigin.Begin);
        ...
        public void CountMaxRect(CharsInfo charsInf)
        {
        ...
        public HYRESULT SetCheckSumAdjustment(string FontFile)
        {
            try
            {
                FRStrm = new FileStream(FontFile, FileMode.Open, FileAccess.ReadWrite,FileShare.None);

                byte[] btFile = new byte[FRStrm.Length];
                FRStrm.Read(btFile, 0, (int)FRStrm.Length);
